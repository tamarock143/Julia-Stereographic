...
SBPSAdaptiveGeom = function(gradlogf, x0, lambda0, T, delta, beta, r, R; Tbrent = 1, Abrent = 1.1, Nbrent = 1, tol = 1e-6,
    sigma = sqrt(length(x0))I(length(x0)), mu = zeros(length(x0)), burnin = 1e2, adaptlength = burnin, forgetrate = 1/2, updategamma = true, updatelambda = false, lambdageom = 1)
    ...
    #Prepare storage for bounce ratio and estimators for lambda
    bounceratio::Vector{Float64} = zeros(nadapt)
    lambda::Vector{Float64} = zeros(nadapt+1)
    lambda[1] = lambda0
    ...
    for t in times
        ...
        @time (zpath,vpath,bounceratio[iadapt],Nevals[iadapt,:],Tbrent) = SBPSGeom(gradlogf, xout[adaptstarts[iadapt],:], lambda[iadapt], min(t,left), delta; 
        w, Tbrent, Abrent, Nbrent, tol, sigma = sigmaest[iadapt], mu = muest[iadapt,:])[(:z, :v, :bounceratio, :Nevals, :Tbrent)]
        ...
        #Update refresh rate lambda via geometric update targeting bounce rate of 0.2188
        if updatelambda
            lambda[iadapt+1] = lambda[iadapt]*exp(lambdageom*(bounceratio[iadapt] - 0.2188)/iadapt)

            #Truncate estimator for theoretical reasons
            #Also, don't want too few refreshes, so we cap lambda at 1/pi to avoid u-turns
            lambda[iadapt+1] > R^2 && (lambda[iadapt+1] = R^2)
            lambda[iadapt+1] < max(1/pi,r^2) && (lambda[iadapt+1] = max(1/pi,r^2))

        else
            lambda[iadapt+1] = lambda0
        end
    ...
    return (x = xout, z = zout, v = vout, mu = muest, sigma = sigmaest, times = times, c = cout, Nevals = Nevals, Tbrent = Tbrent, lambda = lambda, bounceratio = bounceratio)
end


#SBPS with fixed Brent window. Not recommended nor maintained
